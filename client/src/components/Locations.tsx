  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
          ...
          <div style={{ backgroundColor: '#fff', padding: '20px', borderRadius: '8px', width: '400px', boxShadow: '0 4px 8px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <button
                ...
                style={{
                  backgroundColor: (!newLocation.name || !newLocation.siteId || isSubmitting) ? '#ccc' : '#2196F3',
                  color: '#fff',
                  padding: '10px 20px',
                  border: 'none',
                  borderRadius: '4px',
                  cursor: (!newLocation.name || !newLocation.siteId || isSubmitting) ? 'not-allowed' : 'pointer',
                  fontSize: '16px',
                }}
                onMouseOver={(e) => {
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '20px' }}>
        <thead>
          <tr>
            <th style={{ border: '1px solid #ddd', padding: '12px', backgroundColor: '#f5f5f5', color: '#333' }}>Location Name</th>
            <th style={{ border: '1px solid #ddd', padding: '12px', backgroundColor: '#f5f5f5', color: '#333' }}>Site</th>
          </tr>
        </thead>
        <tbody>
          {locations.map(location => (
            <tr key={location.locationId}>
              <td style={{ border: '1px solid #ddd', padding: '12px', textAlign: 'center' }}>{location.name}</td>
              <td style={{ border: '1px solid #ddd', padding: '12px', textAlign: 'center' }}>
                {sites.find(site => site.siteId === location.siteId)?.name || 'Unknown Site'}
              </td>
            </tr>
          ))}
        </tbody>
